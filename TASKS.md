...
- [x] v0.24.0 (Three.js): sétima rodada de ajustes do Caique (só no 3D)
  ...
  - [x] **Salvar do editor**: com senha (EDITOR_KEY no Render) grava `public/demo3d/edits3d.js` na branch `editor` do GitHub (GITHUB_TOKEN no Render); o Claude pega de lá e põe no jogo. Também tem "Copiar código"
  - [x] **Mapa novo Castelo**: castelo de magia no meio (salão com 4 portas e terraço com ameias) e 4 torres redondas com **rampa em espiral** por dentro até uma **sacada sem parapeito** em volta
  - [x] **Mapa novo Navio na tempestade**: navio de bruxos (cabeças de dragão, velas com lua e estrela), mar agitado, chuva, neblina, relâmpago, ilha com vulcão em erupção; o navio **balança e inclina** (escorrega e muda o pulo) e a cada 20 s uma **onda grande de lado** inclina forte; amurada baixinha: caiu no mar, morreu; cabines por dentro dos castelos de popa/proa, 3 mastros com escada de corda até o cesto
  - [x] Metrô: teto mais alto (câmera não passa mais do teto) e **rampa** no lugar da escada. Floresta: **tampa** de volta no alçapão (desce com S em cima dela) e escada inclinada (fora do tronco)
  - [x] Deserto: **uma pirâmide em cada base** (no canto, areia baixa em volta) e montanhas no meio, a mais alta bem no meio. Vulcão: boca 10% menor e nuvens de cinza novas (anéis no shader)
  - [x] Neve: iglu com **3 entradas em triângulo** (sem visão direta de uma pra outra) e sem buraco em volta da porta. Nave: cor dos bonecos voltou ao normal; parede com o painel cinza embaixo e as janelas em cima
  - [x] Furacão novo (funil de vento no shader, bem transparente, sem riscos). Onda do mar: som só quando passa pela plataforma. Tiros dos outros mais baixos e graves
  - [x] Granada sempre na força máxima (sem segurar). Correndo: só não atira nem dá facada (poção e granada pode). Pulo duplo: pegadas mais brilhantes e juntas, sem brilho na bota
  - [ ] Configurar no Render: EDITOR_KEY e GITHUB_TOKEN (ver demo3d/LEIA-ME.md)
  - [ ] Validar no PC (FPS do navio/castelo) e jogar o multiplayer com 2+ pessoas
